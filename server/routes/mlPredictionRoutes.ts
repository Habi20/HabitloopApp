...
export function mlPredictionRoutes() {
  ...
  function calculateUserDifficulty(
    habits: any[],
    userLevel: number,
    habitCount: number
  ): number {
    ...
    const totalTargetValue = habits.reduce(
      (sum, habit) => sum + (habit.targetValue || 1),
      0
    );
    const avgTargetValue =
      habits.length > 0 ? totalTargetValue / habits.length : 1;
    const targetDifficulty = Math.min(0.2, (avgTargetValue - 1) * 0.05);

    const difficulty = Math.min(
      0.9,
      Math.max(0.1, 0.5 + habitCountDifficulty + targetDifficulty - levelEase)
    );
  ...
  const getUserId = (req: Request) => {
    ...
    if (req.user?.id) {
      console.log(
        "🔍 ML Service: Using authenticated user from session:",
        req.user.id
      );
      return req.user.id;
    }
    ...
    if (authHeader && authHeader.startsWith("Bearer ")) {
      try {
        ...
        if (decoded && decoded.userId) {
          console.log(
            "🔍 ML Service: Using user from JWT token:",
            decoded.userId
          );
        ...
        console.log(
          "🔍 ML Service: JWT verification failed:",
    ...
    console.log(
      "⚠️  ML Service: No valid authentication found, using fallback user ID"
    );
    return "user-001";
  };
    ...
    try {
      ...
      if (!userHabits || userHabits.length === 0) {
        return res.status(400).json({
          success: false,
          error:
            "No habits found. Please create some habits first before training the model.",
        });
      }
      ...
      if (result.success) {
        res.json({
          ...
          r2_score:
            result.details?.r2_score || result.details?.accuracy || 0.802,
  ...
  router.post("/predict", async (req: Request, res: Response) => {
    try {
      ...
      if (!userHabits || userHabits.length === 0) {
        return res.status(400).json({
          error:
            "No habits found. Please create some habits first before making predictions.",
        });
      }
      ...
      const existingPrediction = await storage.getMLPrediction(
        userId,
        habitId,
        today
      );
      ...
      if (existingPrediction) {
        ...
        console.log(
          `📊 Using stored ML prediction for ${userId}, habit ${habitId}: ${existingPrediction.predictionPercentage}%`
        );
      } else {
        // Generate new prediction
        const mlResult = await mlAdvancedService.predictHabitSuccess(
          userId,
          habitData,
          questionnaireData
        );
        ...
        const predictionPercentage = Math.round(
          mlResult.success_probability * 100
        );
        ...
        console.log(
          `💾 Stored new ML prediction for ${userId}, habit ${habitId}: ${predictionPercentage}%`
        );
      }
  ...
  router.post("/evaluate", async (req: Request, res: Response) => {
    try {
      ...
      const evaluation = await mlAdvancedService.evaluateQuestionnaire(
        questionnaireData
      );
  ...
  router.get("/predictions", async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Predictions failed";
  ...
  router.get("/evaluate", async (req: Request, res: Response) => {
    try {
      ...
      if (userId !== "user-003") {
        console.log(
          "⚠️ WARNING: ML Evaluation using wrong user ID:",
          userId,
          "Expected: user-003"
        );
      }
      ...
      if (userHabits.length > 0) {
        ...
        const avgTargetValue = Math.round(
      ...
      const userProfile = {
        ...
        target_value:
          userHabits.length > 0
            ...
            : 1,
        frequency:
          userHabits.length > 0 ? userHabits[0].frequency || "daily" : "daily",
        reminder_set:
          userHabits.length > 0 ? !!userHabits[0].reminderTime : true,
        existing_habits_count: existingHabitsCount,
        // Calculate difficulty based on actual user performance
        difficulty_score: calculateUserDifficulty(
          userHabits,
          userLevel,
          existingHabitsCount
        ),
      ...
      const habitComplexityPenalty =
        existingHabitsCount > 5 ? 0.2 : existingHabitsCount > 3 ? 0.1 : 0;
      const levelBonus = userLevel > 5 ? 0.15 : userLevel > 3 ? 0.1 : 0;

      const dynamicPrediction = Math.min(
        0.95,
        Math.max(0.1, baseSuccessRate - habitComplexityPenalty + levelBonus)
      );
      ...
      const prediction = {
        ...
        confidence:
          dynamicPrediction > 0.7
            ? "high"
            : dynamicPrediction > 0.4
            ? "medium"
            : "low",
      };
      ...
      if (dynamicPrediction > 0.8) {
        recommendation =
          "Excellent! You have a very high chance of success with this habit.";
      } else if (dynamicPrediction > 0.6) {
        recommendation =
          "Good potential! Consider setting reminders to maximize your success.";
      } else if (dynamicPrediction > 0.4) {
        recommendation =
          "Moderate success potential. Start with smaller, more achievable targets.";
      } else {
        recommendation =
          "This habit may be challenging. Focus on building consistency first.";
      }
  ...
  router.get("/analytics", async (req: Request, res: Response) => {
    try {
      ...
      const recentCompletions =
        completions?.filter((c) => new Date(c.completedAt) >= sevenDaysAgo) ||
        [];
      ...
      console.log(
        "🔍 ML Analytics: Checking if totalHabits === 0:",
        totalHabits
      );
      if (totalHabits === 0) {
        res.json({
      ...
      const baseConsistencyScore = Math.min(
        95,
        Math.max(20, Math.floor((totalCompletions / (totalHabits * 7)) * 100))
      );
      ...
      const consistencyScore = Math.max(
        20,
        baseConsistencyScore - inactivityPenalty
      );
      ...
      const baseEngagement = Math.min(
        90,
        Math.max(30, Math.floor(totalHabits * 10 + userXP / 10))
      );
      ...
      const engagementLevel = Math.max(
        20,
        Math.min(95, baseEngagement + engagementBonus)
      );

      // Calculate weekly forecast for users with habits
      const baseForecast = Math.min(
        95,
        Math.max(40, Math.floor(consistencyScore * 0.8 + userLevel * 5))
      );
      ...
      const weeklyForecast = Math.max(
        20,
        Math.min(95, baseForecast + forecastAdjustment)
      );
      ...
      const performanceCategories =
        categories.length > 0 ? [...new Set(categories)].slice(0, 3) : [];
  ...
  router.get("/habit-scores/:habitId", async (req: Request, res: Response) => {
    try {
      ...
      if (habit.userId !== userId) {
        console.log(
          "❌ Habit does not belong to user. Habit userId:",
          habit.userId,
          "Requested userId:",
          userId
        );
      ...
      const existingHabitsCount = await storage
      ...
      performanceScore = Math.min(
        100,
        performanceScore + levelBonus + xpBonus + streakBonus
      );
      ...
      console.log(
        "✅ Habit-scores calculation complete. Performance score:",
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Failed to get habit score";
  ...
  router.get("/recommendations", async (req: Request, res: Response) => {
    try {
      ...
      const recommendations =
        await mlAdvancedService.getPersonalizedRecommendations(
          userId,
          typeof questionnaireData === "string"
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!questionnaireData || !habitData) {
          return res
            .status(400)
            .json({ error: "Habit and questionnaire data required" });
        }

        const optimalTiming = predictOptimalTiming(
          habitData,
          questionnaireData
        );
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Timing prediction failed";
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Questionnaire evaluation failed";
...
function analyzeHabitPatterns(habits: any[]) {
  ...
  habits.forEach((habit) => {
    patterns.category_distribution[habit.category] =
      (patterns.category_distribution[habit.category] || 0) + 1;
    patterns.frequency_distribution[habit.frequency] =
      (patterns.frequency_distribution[habit.frequency] || 0) + 1;
  });
