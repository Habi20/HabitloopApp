...
export function filterHabitsByProfile(questionnaire: Questionnaire): SyntheticHabit[] {
  try {
    console.log("Filtering habits by profile:", JSON.stringify(questionnaire, null, 2));
    
    let filteredHabits = [...syntheticHabitDatabase];
    console.log("Initial habits count:", filteredHabits.length);

    // Filter by focus areas
    if (questionnaire.focusAreas && questionnaire.focusAreas.length > 0) {
      console.log("Filtering by focus areas:", questionnaire.focusAreas);
      const focusFiltered = filteredHabits.filter(habit => 
      ...
      // If focus filtering returns no results, don't apply this filter
      if (focusFiltered.length > 0) {
        filteredHabits = focusFiltered;
        console.log("After focus areas filter:", filteredHabits.length);
      } else {
        console.log("Focus areas filter returned no results, skipping this filter");
      }
    }

    // Filter by motivation time preference
    if (questionnaire.motivationTime) {
      console.log("Filtering by motivation time:", questionnaire.motivationTime);
      ...
      const timeFiltered = filteredHabits.filter(habit => 
        preferredTimes.includes(habit.reminderTime)
      );
      
      // If time filtering returns no results, don't apply this filter
      if (timeFiltered.length > 0) {
        filteredHabits = timeFiltered;
        console.log("After motivation time filter:", filteredHabits.length);
      } else {
        console.log("Motivation time filter returned no results, skipping this filter");
      }
    }

    // Filter by consistency rating (difficulty preference)
    if (questionnaire.consistencyRating) {
      console.log("Filtering by consistency rating:", questionnaire.consistencyRating);
      const difficultyMap = {
        1: ["easy"],
        2: ["easy"],
        3: ["easy", "medium"],
      ...
      const difficultyFiltered = filteredHabits.filter(habit => 
        allowedDifficulties.includes(habit.difficulty)
      );
      
      // If difficulty filtering returns no results, don't apply this filter
      if (difficultyFiltered.length > 0) {
        filteredHabits = difficultyFiltered;
        console.log("After consistency rating filter:", filteredHabits.length);
      } else {
        console.log("Consistency rating filter returned no results, skipping this filter");
      }
    }
    ...
    const result = filteredHabits.slice(0, 10);
    console.log("Final filtered habits count:", result.length);
    
    // If we still have no results, return some default habits
    if (result.length === 0) {
      console.log("No habits found after filtering, returning default habits");
      return syntheticHabitDatabase.slice(0, 10);
    }
    
    return result;
  } catch (error) {
    console.error("Error in filterHabitsByProfile:", error);
    // Return some default habits if there's an error
    return syntheticHabitDatabase.slice(0, 10);
  }
}
...
export function generatePersonalizedHabits(questionnaire: Questionnaire): SyntheticHabit[] {
  try {
    ...
    if (baseHabits.length < 10) {
      console.log("Adding fallback habits to reach minimum of 10");
    ...
    console.log("Final habits count:", baseHabits.length);
    return baseHabits;
  } catch (error) {
    console.error("Error in generatePersonalizedHabits:", error);
    // Return some default habits if there's an error
    return syntheticHabitDatabase.slice(0, 10);
  }
}