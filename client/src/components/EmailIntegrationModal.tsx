...
export function EmailIntegrationModal({ open, onClose }: EmailIntegrationModalProps) {
  ...
  const connectEmailMutation = useMutation<ConnectResponse, Error>({
    mutationFn: async () => {
    ...
    onSuccess: (data: ConnectResponse) => {
      if (data.authUrl) {
    ...
    onError: (error) => {
      ...
      toast({
        ...
        description: error.message.includes('credentials')
          ? "Missing Google API credentials - check server configuration"
          : error.message,
        variant: "destructive",
      });
    },
  ...
  const updateSettingsMutation = useMutation<void, Error, EmailSettings>({
  ...
  const sendTestEmailMutation = useMutation<void, Error>({
    mutationFn: async () => {
                  ...
                  {emailTypes.map((emailType) => {
                    ...
                    return (
                      <div key={emailType.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                        ...
                        <Switch
                          checked={emailSettings[emailType.id as keyof EmailSettings]}
