...
export function GoogleCalendarIntegrationSimple() {
  ...
  const calendarMutation = useMutation({
    mutationFn: async (habitData: any) => {
      ...
      const predictionResponse = await apiRequest('/api/ml/predict', 'POST', {
        user_level: 5,
        user_xp: 1200,
      ...
      const calendarResponse = await apiRequest('/api/integrations/google-calendar', 'POST', {
    ...
    onSuccess: (data) => {
      const newIntegration: CalendarIntegration = {
        ...
        success_probability: data.prediction.interpretation?.success_probability || '0%',
        confidence: data.prediction.interpretation?.confidence_level || 'medium',
