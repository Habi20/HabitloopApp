...
export function HabitLoopLoginModal({ 
  ...
  const fetchUserProfile = async (userId: string) => {
    ...
    try {
      const response = await fetch(buildApiUrl(`habitloop/user-profile/${userId}`), {
        method: 'GET',
      ...
      if (response.ok) {
        ...
        // Server error - show warning but don't block
        console.warn('Server error fetching user profile:', response.status);
        setUserProfile(null);
      } else {
        setUserProfile(null);
      }
    } catch (error) {
      // Network failure - show warning but don't block
      console.warn('Network error fetching user profile:', error);
      setUserProfile(null);
    }
  };

  // Debounced user profile fetch
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await loginAsHabitLoopUser({ userId, password });
