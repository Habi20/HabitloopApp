...
export function HabitCard({
  ...
  const [performanceScore, setPerformanceScore] =
    useState<HabitPerformanceScore | null>(null);
      ...
      <CardContent className="p-3 sm:p-4">
        <div className="flex items-start space-x-3 sm:space-x-4 md:space-x-5 lg:space-x-6">
          <button
            ...
            className={cn(
              buttonClasses,
              "flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 lg:w-16 lg:h-16"
            )}
          >
            {completed ? (
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-2 mb-1 space-y-1 sm:space-y-0">
              <h4 className="font-semibold text-gray-900 text-sm sm:text-base md:text-lg lg:text-xl truncate">
                {habit.title}
              </h4>
              <Badge
                className={cn(
            ...
            {habit.description && (
              <p className="text-gray-600 text-xs sm:text-sm md:text-base lg:text-lg mb-2 line-clamp-2">
                {habit.description}
              </p>
            )}
              ...
              <div className="mt-2 flex items-center justify-between text-xs">
                <div className="flex items-center space-x-2">
                  ...
                  <span
                    className={cn(
                      "font-semibold",
                      performanceScore.performance_score >= 80
                        ? "text-green-600"
                        : performanceScore.performance_score >= 50
                        ? "text-yellow-600"
                        : "text-red-600"
                    )}
                  >
